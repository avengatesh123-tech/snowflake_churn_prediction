-- Create Warehouse
CREATE
OR REPLACE WAREHOUSE ML_WH WAREHOUSE_SIZE = 'MEDIUM' AUTO_SUSPEND = 300 AUTO_RESUME = TRUE;
USE WAREHOUSE ML_WH;
-- Create Database and Schemas
CREATE
OR REPLACE DATABASE TELCO_ML;
USE DATABASE TELCO_ML;
CREATE
OR REPLACE SCHEMA RAW;
CREATE
OR REPLACE SCHEMA FEATURE;
CREATE
OR REPLACE SCHEMA ML;
-- Create Stage and File Format
CREATE
OR REPLACE STAGE TELCO_STAGE;
CREATE
OR REPLACE FILE FORMAT TELCO_CSV_FORMAT TYPE = CSV FIELD_DELIMITER = ',' SKIP_HEADER = 1 NULL_IF = ('');
-- Create Raw Table
CREATE
OR REPLACE TABLE RAW.TELCO_CUSTOMER_CHURN (
    customerID STRING,
    gender STRING,
    SeniorCitizen INT,
    Partner STRING,
...
FROM
    @TELCO_ML.ML.TELCO_STAGE FILE_FORMAT = TELCO_CSV_FORMAT ON_ERROR = 'CONTINUE';
LIST @TELCO_ML.ML.TELCO_STAGE;
COPY INTO RAW.TELCO_CUSTOMER_CHURN
FROM
    @TELCO_STAGE FILE_FORMAT = TELCO_CSV_FORMAT ON_ERROR = 'CONTINUE';
-- Feature Engineering
    CREATE
    OR REPLACE TABLE FEATURE.TELCO_FEATURES AS
SELECT
    customerID,
    SeniorCitizen,
    tenure,
    ...
    END AS churn_label
FROM
    RAW.TELCO_CUSTOMER_CHURN
WHERE
    TotalCharges IS NOT NULL;
-- Train/Test Split
    CREATE
    OR REPLACE TABLE ML.TRAIN_DATA AS
SELECT
    *
FROM
    FEATURE.TELCO_FEATURES SAMPLE (80);
CREATE
    OR REPLACE TABLE ML.TEST_DATA AS
SELECT
    *
FROM
    FEATURE.TELCO_FEATURES MINUS
SELECT
    *
FROM
    ML.TRAIN_DATA;
...
SELECT
    ...
    GET(
        ...
        prediction_results:class::STRING
